    ...
    extends FMLIndexedMessageToMessageCodec<IPacket>
{
    ...
    public boolean registerPacket(Class<? extends IPacket> clazz) {
        if (this.packets.size() > 256) {
            FMLLog.log(this.logger, Level.ERROR, "Cannot register more than 256 packets! Packet %s not registered!",
                       clazz.getName());
            return false;
        }
        ...
        if (this.isPostInitialised) {
            FMLLog.log(this.logger, Level.ERROR, "Packet %s cannot be registered during / after Post-Initialization!",
                       clazz.getName());
            return false;
        }
